function graphs_in_window_spectral_fun(filename, wind, tstep, thr_fr, crp_emb_dimm, crp_delay, crp_epsilon, logfile, graph_type, docalcentr, p, k, beta, n_pref, m_pref, doplot_ipr, rmt_multiplier, filename_index,adjmatrixfile);
%!!!! graphs_spectrum_cmd_ts - для ряда, а graphs_spectrum_cmd_model - для
...
for i=1:tstep:n-wind % главный цикл
    ...
    if ((model_type==11) || (model_type==12))
        Adj=data_to_graph(adjmatrixfile, model_type, t_beg, t_end, wind, p, k, beta, n_pref, m_pref, crp_emb_dimm, crp_delay, crp_epsilon, rmt_multiplier);
    else
...
open_all_eigenvectors(logfile,filename_index,wind,tstep,doplot_ipr);
