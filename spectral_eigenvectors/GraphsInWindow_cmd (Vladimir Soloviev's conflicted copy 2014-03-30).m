...
%filename='world.txt';
outfile='ux_04.csv';
logfile='ux_04_vg_250_1_topological_norm2.txt';
filename_index='dax_04.txt'; % имя файла временного ряда индекса. В случае мер crp, lacasa должен равняться filename
...
type_normalize =1; % 1 - нормализованный лапласиан вместо обычного 0 - обычный. % type_normalize=0 - non-normalized; 1 - normalized ;2 - nlaplacian 
% Нормализация: 
if (measure_type==1)
    ...
    graphs_in_window_spectral_fun_norm2(filename, wind, tstep, thr_fr, crp_emb_dimm, crp_delay, crp_epsilon, logfile, model_type, docalcentr, p, k, beta, n_pref, m_pref,doplot_ipr, crp_multiplier,filename_index, type_normalize);
end
