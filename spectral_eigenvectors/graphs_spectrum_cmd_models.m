%!!!! graphs_spectrum_cmd - для ряда, а graphs_spectrum_cmd_model - для
%моделей (случайных графов)!!!
% Вид и параметры моделей задаются в цикле (раскомментировать нужную
% модель)
filename='ux_all.txt';
y=dlmread(filename);
n=length(y);
...
crp_emb_dimm=1;
crp_delay=1;
crp_epsilon=0.3; %0.1 - Marwan default
logfile='Star_spec_250_5.txt';
...
for i=1:tstep:n-wind % главный цикл
    ...
    %end
    %Adj=random_graph(wind); % Erdos Renyi with p=0.5
    % parameters for random graph (Renyi):p
    % p=0.5;
    % Adj=random_graph(wind,p); % Erdos Renyi with the given p;
    %'line','circle','star','btree','tree','htree','trilattice','sqlattice','hexlattice', 'clique'
    ...
    % n - final (desired) number of vertices, m - # edges to attach at every step
     % n = wind;
     % m = 5; % кол-во ребер, добавляемых каждый раз.
    %  edgel = preferential_attachment(n,m);
    %edgel=exponential_growth_model(50);
    % parameters for newmangastner: beta, plt
    %beta=0.9; %0t 0 do 1
    %plt='off';% plot on|off;
    %[edgel,points]=newmangastner(wind,beta,plt);
    ...
    % fprintf
    [gr_sp,gr_sp_vect]=graph_spectrum_unsort(Adj);
...
open_all_eigenvectors(logfile,filename,wind,tstep);